...
for file in os.listdir(pathIn):
    ...
    if face != None:
        ...
        for i, data in enumerate(dataset):
            ...
            image_numpy_fakeA = imresize(tensor2im(visuals["fake_A"]), (square_length, square_length), interp='bicubic')
            image_numpy_recB = imresize(tensor2im(visuals["rec_B"]), (square_length, square_length), interp='bicubic')

            image_numpy_fakeA = cv2.cvtColor(image_numpy_fakeA, cv2.COLOR_RGB2BGR)
            image_numpy_recB = cv2.cvtColor(image_numpy_recB, cv2.COLOR_RGB2BGR)
            ...
            numpy_horizontal = np.hstack((resized_img, image_numpy_fakeA, image_numpy_recB))
